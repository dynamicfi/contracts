...
async function main() {
  ...
  const DyBTCVenus = await hre.ethers.getContractFactory("DyBEP20VenusProxy");
  const dyBTCVenus = await upgrades.deployProxy(DyBTCVenus, [
  ...
  await dyBTCVenus.deployed();

  console.log("DyUSDTVenus deployed to:", dyBTCVenus.address);
}
